...
namespace doyou {
	namespace io {
		class TcpIocpClient : public TcpClient
		{
		public:
		...
		protected:
			int DoIocpNetEvents(int microseconds)
			{
				...
				if (ret < 0)
				{
					CELLLog_Error("TcpIocpClient.DoIocpNetEvents.wait clientId<%d> sockfd<%d>", _pClient->id, (int)_pClient->sockfd());
					return ret;
				}
				else if (ret == 0)
				{
				...
				if (IO_TYPE::RECV == _ioEvent.pIoData->iotype)
				{
					...
					{//客户端断开处理
						CELLLog_Error("TcpIocpClient.DoIocpNetEvents rmClient sockfd=%d, IO_TYPE::RECV bytesTrans=%d", _ioEvent.pIoData->sockfd, _ioEvent.bytesTrans);
						Close();
						return -1;
					}
					//
				...
				else if (IO_TYPE::SEND == _ioEvent.pIoData->iotype)
				{
					if (_ioEvent.bytesTrans <= 0)
					{//客户端断开处理
						CELLLog_Error("TcpIocpClient.DoIocpNetEvents rmClient sockfd=%d, IO_TYPE::SEND bytesTrans=%d", _ioEvent.pIoData->sockfd, _ioEvent.bytesTrans);
						Close();
						return -1;
					}
					//
